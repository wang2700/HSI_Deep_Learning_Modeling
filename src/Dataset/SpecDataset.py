...
class SpecDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        whiteRef = np.load(self.root_dir + r'/' +
                           self.rawNameList[idx] + '_whRef.npy')
        pst = np.load(self.root_dir + r'/' +
                      self.rawNameList[idx] + '_imgPst.npy')
        # white reference calibration
        image = caliColor(image, whiteRef)
        # pad/cut image to the same size
        if (image.shape[1] < self.max_length):
            image = np.pad(
                image, ((0, 0), (self.max_length - image.shape[1], 0), (0, 0)))
        else:
        ...
        return {'hsi_img': torch.tensor(image, dtype=torch.float32), 'mask': mask, 'pst': pst, 'shape': image.shape, 'gt': gt}
